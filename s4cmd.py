...
import sys, os, re, optparse, multiprocessing, fnmatch, time, hashlib, errno, \
    pytz
import logging, traceback, types, threading, random, socket, shlex, datetime, \
    json
  ...
  '''
    ...
    def __init__(self, opt=None):
        ...
        for o in parser.option_list:
            self.__dict__[o.dest] = o.default if (opt is None) or (
            opt.__dict__[o.dest] is None) else opt.__dict__[o.dest]
...
class S4cmdLoggingClass:
    ...
    def configure(self, opt):
        ...
        self.log_handler.setFormatter(
            logging.Formatter('%(message)s', DATETIME_FORMAT))
        if opt.debug:
...
def log_calls(func):
    ...
    def wrapper(*args, **kargs):
        callStr = "%s(%s)" % (func.__name__, ", ".join(
            [repr(p) for p in args] + ["%s=%s" % (k, repr(v)) for (k, v) in
                                       list(kargs.items())]))
        debug(">> %s", callStr)
        ret = func(*args, **kargs)
...
def tempfile_get(target):
    ...
    fn = '%s-%s.tmp' % (target, ''.join(
        random.Random().sample("0123456789abcdefghijklmnopqrstuvwxyz", 15)))
    TEMP_FILES.add(fn)
    return fn
...
class S3URL:
  ...
  '''
    ...
    def __init__(self, uri):
        ...
        try:
            self.proto, self.bucket, self.path = S3URL.S3URL_PATTERN.match(
                uri).groups()
  ...
  '''
    ...
    def __getattribute__(self, method):
        ...
        if method in BotoClient.ALLOWED_CLIENT_METHODS:
            def wrapped_method(*args, **kargs):
                ...
                callStr = "%s(%s)" % ("S3APICALL " + method, ", ".join(
                    [repr(p) for p in args] + ["%s=%s" % (k, repr(v)) for (k, v)
                                               in list(kargs.items())]))
                debug(">> %s", callStr)
    ...
    def add_options(parser):
        ...
        for param, param_type, param_doc in BotoClient.EXTRA_CLIENT_PARAMS:
            parser.add_option('--API-' + param, help=param_doc, type=param_type,
                              dest=param)
...
class ThreadPool(object):
  ...
  '''
            ...
            while True:
                ...
                try:
                    ...
                    if retry >= self.opt.retry:
                        self.pool.tasks.terminate(e)
                        fail('[Runtime Exception] ', exc_info=e,
                             stacktrace=True)
                    else:
    ...
    def processed(self):
        ...
        if qsize > 0:
            progress('[%d task(s) completed, %d remaining, %d thread(s)]',
                     self.processed_tasks, qsize, len(self.workers))
        else:
            progress('[%d task(s) completed, %d thread(s)]',
                     self.processed_tasks, len(self.workers))
  ...
  '''
    ...
    def s3_keys_from_s3cfg(opt):
        ...
        try:
            ...
            keys = config.get("default", "access_key"), config.get("default",
                                                                   "secret_key")
            debug("read S3 keys from $HOME/.s3cfg file")
            return keys
        except Exception as e:
            info("could not read S3 keys from %s file; skipping (%s)",
                 s3cfg_path, e)
            return None
    ...
    def init_s3_keys(opt):
        ...
        S3Handler.S3_KEYS = S3Handler.s3_keys_from_cmdline(
            opt) or S3Handler.s3_keys_from_env() \
    ...
    def connect(self):
        ...
        try:
            if S3Handler.S3_KEYS:
                self.s3 = BotoClient(self.opt, S3Handler.S3_KEYS[0],
                                     S3Handler.S3_KEYS[1])
            else:
    ...
    def put_single_file(self, pool, source, target):
        ...
        if os.path.isdir(source):
            if self.opt.recursive:
                for f in (f for f in self.local_walk(source) if
                          not os.path.isdir(f)):
                    target_url = S3URL(target)
                    # deal with ./ or ../ here by normalizing the path.
                    joined_path = os.path.normpath(os.path.join(target_url.path,
                                                                os.path.relpath(
                                                                    f, source)))
                    pool.upload(f, S3URL.combine('s3', target_url.bucket,
                                                 joined_path))
            else:
    ...
    '''
        ...
        if target[-1] == PATH_SEP:
            for src in source:
                self.put_single_file(pool, src, os.path.join(target,
                                                             self.get_basename(
                                                                 src)))
        else:
            if len(source) == 1:
                ...
                raise Failure(
                    'Target "%s" is not a directory (with a trailing slash).' % target)
        ...
        if source[-1] == PATH_SEP:
            if self.opt.recursive:
                ...
                for f in (f for f in self.s3walk(source) if not f['is_dir']):
                    pool.download(f['name'], os.path.join(target,
                                                          os.path.relpath(S3URL(
                                                              f['name']).path,
                                                                          basepath)))
            else:
                message('omitting directory "%s".' % source)
    ...
    '''
        ...
        if os.path.isdir(target):
            for src in source:
                self.get_single_file(pool, src, os.path.join(target,
                                                             self.get_basename(
                                                                 S3URL(
                                                                     src).path)))
        else:
            if len(source) > 1:
                ...
                # Source expand may return empty list only if
                # ignore-empty-source is set to true
                pass
    ...
    '''
        ...
        if os.path.isdir(source):
            ...
            for f in [f for f in self.s3walk(target) if not f['is_dir']]:
                local_name = os.path.join(source,
                                          os.path.relpath(S3URL(f['name']).path,
                                                          basepath))
                if not os.path.isfile(local_name):
                    message("%s not found locally, adding to delete queue",
                            local_name)
    ...
    def cp_single_file(self, pool, source, target, delete_source):
        ...
        if source[-1] == PATH_SEP:
            if self.opt.recursive:
                basepath = S3URL(source).path
                for f in (f for f in self.s3walk(source) if not f['is_dir']):
                    pool.copy(f['name'], os.path.join(target, os.path.relpath(
                        S3URL(f['name']).path, basepath)),
    ...
    '''
        ...
        if target[-1] == PATH_SEP:
            for src in source:
                self.cp_single_file(pool, src, os.path.join(target,
                                                            self.get_basename(
                                                                S3URL(
                                                                    src).path)),
                                    delete_source)
    ...
    def dsync_files(self, source, target):
        ...
        sync_list = [(os.path.join(source, f), os.path.join(target, f)) for f in
                     source_list]
        ...
        if self.opt.delete_removed:
            ...
            remove_list = [os.path.join(target, f) for f in
                           (set(target_list) - set(source_list))]
...
class ThreadUtil(S3Handler, ThreadPool.Worker):
  ...
  '''
    ...
    '''
        ...
        return ('ETag' in remoteKey and remoteKey[
            'ETag'] == '"%s"' % localmd5) or \
               ('md5' in remoteKey and remoteKey['md5'] == localmd5) or \
               ('md5' in remoteKey['Metadata'] and remoteKey['Metadata'][
                   'md5'] == localmd5)
    ...
    '''
        ...
        matched = fnmatch.fnmatch(PATH_SEP.join(pi[0:min_len]),
                                  PATH_SEP.join(fi[0:min_len]))
    ...
    '''
        ...
        for page in paginator.paginate(Bucket=s3url.bucket, Prefix=s3dir,
                                       Delimiter=PATH_SEP,
            ...
            for obj in page.get('CommonPrefixes') or []:
                ...
                if self.opt.recursive or (
                    obj_name.count(PATH_SEP) != filter_path_level + 1):
                    self.pool.s3walk(s3url, obj_name, filter_path, result)
                else:
                    self.conditional(result, {
                        'name': S3URL.combine(s3url.proto, s3url.bucket,
                                              obj_name),
                        'is_dir': True,
                        'size': 0,
                        'last_modified': None
            ...
            for obj in page.get('Contents') or []:
                ...
                if self.opt.recursive or obj_name.count(
                        PATH_SEP) == filter_path_level:
                    self.conditional(result, {
                        'name': S3URL.combine(s3url.proto, s3url.bucket,
                                              obj_name),
                        'is_dir': False,
    ...
    def conditional(self, result, obj):
        ...
        fileonly = (self.opt.last_modified_before is not None) or (
        self.opt.last_modified_after is not None)
        ...
        if (self.opt.last_modified_before is not None) and obj[
            'last_modified'] >= self.opt.last_modified_before:
            return

        if (self.opt.last_modified_after is not None) and obj[
            'last_modified'] <= self.opt.last_modified_after:
            return
    ...
    def get_file_privilege(self, source):
        ...
        try:
            ...
            raise Failure('Could not get stat for %s, error_message = %s',
                          source, e)
    ...
    def upload(self, source, target, mpi=None, pos=0, chunk=0, part=0):
        ...
        if not mpi:
            ...
            for args in self.get_file_splits(upload_id, source, target, fsize,
                                             self.opt.multipart_split_size):
                self.pool.upload(*args)
            return

        data = self.read_file_chunk(source, pos, chunk)
        response = self.s3.upload_part(Bucket=s3url.bucket, Key=s3url.path,
                                       UploadId=mpi.id, Body=data,
                                       PartNumber=part)

        # Finalize
        if mpi.complete({'ETag': response['ETag'], 'PartNumber': part}):
            try:
                self.s3.complete_multipart_upload(Bucket=s3url.bucket,
                                                  Key=s3url.path,
                                                  UploadId=mpi.id,
                                                  MultipartUpload={
                                                      'Parts': mpi.sorted_parts()})
                message('%s => %s', source, target)
            except Exception as e:
                message('Unable to complete upload: %s', str(e))
                self.s3.abort_multipart_upload(Bucket=s3url.bucket,
                                               Key=s3url.path, UploadId=mpi.id)
                raise RetryFailure(
                    'Upload failed: Unable to complete upload %s.' % source)
    ...
    def _verify_file_size(self, obj, downloaded_file):
        ...
        if int(obj['ContentLength']) != file_size:
            raise RetryFailure(
                'Downloaded file size inconsistent: %s' % (repr(obj)))
    ...
    def download(self, source, target, mpi=None, pos=0, chunk=0, part=0):
        ...
        if not mpi:
            ...
            if self.opt.dry_run:
                message('%s => %s', source, target)
                return
            elif self.opt.sync_check and self.sync_check(LocalMD5Cache(target),
                                                         obj):
                message('%s => %s (synced)', source, target)
                return
            ...
            if fsize < self.opt.max_singlepart_download_size:
                ...
                for args in self.get_file_splits(tempfile_get(target), source,
                                                 target, fsize,
                                                 self.opt.multipart_split_size):
        ...
        if mpi.complete({'PartNumber': part}):
            try:
                ...
                raise Failure(
                    'Download Failure: %s, Source: %s.' % (e.message, source))

    @log_calls
    def copy(self, source, target, mpi=None, pos=0, chunk=0, part=0,
             delete_source=False):
        ...
        if not mpi:
            ...
            if fsize < self.opt.max_singlepart_copy_size:
                self.s3.copy_object(Bucket=target_url.bucket,
                                    Key=target_url.path,
                                    CopySource={'Bucket': source_url.bucket,
                                                'Key': source_url.path})
        ...
        if mpi.complete({'ETag': response['CopyPartResult']['ETag'],
                         'PartNumber': part}):
            try:
                # Finalize copy operation.
                self.s3.complete_multipart_upload(Bucket=target_url.bucket,
                                                  Key=target_url.path,
                                                  UploadId=mpi.id,
                                                  MultipartUpload={
                                                      'Parts': mpi.sorted_parts()})
                ...
                message('Unable to complete upload: %s', str(e))
                self.s3.abort_multipart_upload(Bucket=source_url.bucket,
                                               Key=source_url.path,
                                               UploadId=mpi.id)
                raise RetryFailure(
                    'Copy failed: Unable to complete copy %s.' % source)
    ...
    def batch_delete(self, sources):
        ...
        if len(sources) == 0:
            ...
            for i in range(0, len(sources), self.opt.batch_delete_size):
                self.pool.batch_delete(
                    sources[i:i + self.opt.batch_delete_size])
        else:
            ...
            for source in sources:
                ...
                if s3url.bucket != bucket:
                    raise Failure(
                        'Unable to delete keys in different bucket %s and %s.' % (
                        s3url.bucket, bucket))
                deletes.append({'Key': s3url.path})

            response = self.s3.delete_objects(Bucket=bucket,
            ...
            for err in response.get('Errors') or []:
                message('Error deleting %s, code(%s) %s',
                        S3URL.combine('s3', bucket, res['Key']), err['Code'],
                        err['Message'])

            if response.get('Errors') is not None:
                raise RetryFailure(
                    'Unable to complete deleting %d files.' % len(
                        response.get('Errors')))
  ...
  '''
    ...
    '''
        ...
        for i, fmt in enumerate(fmts):
            ...
            for f in fmt.split(','):
                if f == 'cmd' and args[
                    i] + '_handler' in CommandHandler.__dict__:
                    valid = True
                ...
                if f == 'local' and not S3URL.is_valid(args[i]):
                    valid = True
            if not valid:
                raise InvalidArgument('Invalid parameter: %s, %s expected' % (
                args[i], fmtMap[fmt.split(',')[0]]))
    ...
    def put_handler(self, args):
        ...
        if len(args) < 3:
            raise InvalidArgument('Invalid number of parameters')
        self.validate('|'.join(['cmd'] + ['local'] * (len(args) - 2) + ['s3']),
                      args)
...
class ExtendedOptParser(optparse.Option):
    ...
    REGEX_DELTA = re.compile(
        r'(\d{1,3})\s+(minute|hour|day|week)s?\s+(ago|before|after)')
    ...
    def check_datetime(self, opt, value):
        ...
        return pytz.utc.localize(
            datetime.datetime.combine(current_date, current_time) + delta)
    ...
    def check_dict(self, opt, value):
        ...
        try:
            ...
            raise optparse.OptionValueError(
                "Option %s: invalid dict value: %r" % (opt, value))

    def check_mimetype(option, opt, value):
        if re.compile("^[a-z0-9]+/[a-z0-9+\.-]+(;.*)?$",
                      re.IGNORECASE).match(value):
            return value
        raise optparse.OptionValueError(
            "option %s: invalid MIME-Type format: %r" % (opt, value))
...
if __name__ == '__main__':
    if not sys.argv[0]: sys.argv[
        0] = ''  # Workaround for running with optparse from egg
    ...
    env_opts = (shlex.split(
        os.environ[S4CMD_ENV_KEY]) if S4CMD_ENV_KEY in os.environ else [])
    ...
    if S3Handler.S3_KEYS is None:
        fail('[Invalid Argument] access key or secret key is not provided ',
             status=-1)
    try:
